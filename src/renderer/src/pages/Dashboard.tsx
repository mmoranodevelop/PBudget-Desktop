...
export default function Dashboard({ categories, onNavigate, onDataChange }: { categories: Category[]; onNavigate: (p: Page) => void; onDataChange: () => void }): JSX.Element {
  ...
  const load = (): void => {
    api.accountList().then((r) => {
      setAccounts(r)
      const fallback = r.find((a) => a.type === 'main')?.id ?? r[0]?.id ?? null
      setSelectedAccountId((id) => id ?? fallback)
      setAccountId((id) => id ?? fallback)
    }).catch(() => undefined)
  }
  useEffect(load, [])
  useEffect(() => {
    if (selectedAccountId == null) return
    api.dashboard(year, selectedAccountId).then(setStats).catch(() => undefined)
    api.txList({ accountIds: [selectedAccountId], sortBy: 'dateReg', sortDir: 'desc', limit: 8 }).then((r) => setRecent(r.rows)).catch(() => undefined)
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-border/80 bg-card/70 p-5 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2"><Select value={selectedAccountId != null ? String(selectedAccountId) : ''} onValueChange={(v) => setSelectedAccountId(Number(v))}><SelectTrigger size="sm" className="w-52"><SelectValue placeholder="Scegli conto o carta" /></SelectTrigger><SelectContent>{accounts.map((a) => <SelectItem key={a.id} value={String(a.id)}>{a.type === 'credit_card' ? 'Carta · ' : a.type === 'secondary' ? 'Conto secondario · ' : 'Conto principale · '}{a.name}</SelectItem>)}</SelectContent></Select><Select value={String(year)} onValueChange={(v) => setYear(Number(v))}><SelectTrigger size="sm" className="w-24"><SelectValue /></SelectTrigger><SelectContent>{[0, 1, 2, 3].map((d) => { const value = new Date().getFullYear() - d; return <SelectItem key={value} value={String(value)}>{value}</SelectItem> })}</SelectContent></Select><Button variant="outline" size="sm" onClick={() => onNavigate('import')}><FileUp className="size-4" />Importa</Button><Button size="sm" onClick={() => { setAccountId(selectedAccountId); setTransactionOpen(true) }}><Plus className="size-4" />Aggiungi movimento</Button></div>
    </div>
